...
class Reconstructor(nn.Module):
    ...
    def forward(self, h):
        h_num  = h[:, :self.d_numerical] if self.d_numerical > 0 else None
        h_cat  = h[:, self.d_numerical:] if self.categories else None
...
class TabularVAE(nn.Module):
    def __init__(self, num_numerical: int, cat_cardinalities: list, latent_dim: int, device: torch.device):
        super().__init__()
        self.num_numerical = num_numerical
        self.cat_cardinalities = cat_cardinalities
        self.device = device
        
        self.d_token = latent_dim 
        self.n_tokens = 1 + num_numerical + len(cat_cardinalities)
        self.flat_latent_dim = self.n_tokens * self.d_token

        self.model = Model_VAE(
            num_layers=4,
            d_numerical=num_numerical,
            categories=cat_cardinalities if cat_cardinalities else None,
    ...
    def _split_and_prepare(self, x: torch.Tensor):

        x_num = x[:, :self.num_numerical] if self.num_numerical > 0 else None
        x_cat_ohe = x[:, self.num_numerical:]
        
        x_cat_indices = []
        start = 0
        for card in self.cat_cardinalities:
            col_ohe = x_cat_ohe[:, start:start+card]
            idx = torch.argmax(col_ohe, dim=1) 
            x_cat_indices.append(idx.unsqueeze(1))
            start += card
            
        if len(x_cat_indices) > 0:
            x_cat = torch.cat(x_cat_indices, dim=1).long()
        else:
            x_cat = None
    ...
    def forward(self, x):
        x_num, x_cat = self._split_and_prepare(x)
        return self.model(x_num, x_cat)

    def encode(self, x):
        x_num, x_cat = self._split_and_prepare(x)
        x_tok = self.model.VAE.Tokenizer(x_num, x_cat)
        mu_z = self.model.VAE.encoder_mu(x_tok)
        return mu_z.view(x.shape[0], -1)

    def decode(self, z):
        B = z.shape[0]
        z_seq = z.view(B, self.n_tokens, self.d_token)
        h = self.model.VAE.decoder(z_seq[:, 1:])
        recon_num, recon_cats = self.model.Reconstructor(h)
        res = []
        if recon_num is not None:
            res.append(recon_num)
        if recon_cats is not None:
            for rc in recon_cats:
                res.append(F.log_softmax(rc, dim=1))
        
        return torch.cat(res, dim=1)

    def loss_function(self, recon_num, recon_cats, x, mu, std_z):
        x_num, x_cat = self._split_and_prepare(x)
        if x_num is not None:
            loss_num = F.mse_loss(recon_num, x_num, reduction='mean')
        else:
            loss_num = torch.tensor(0.0, device=x.device)
        loss_cat = 0.0
        if x_cat is not None:
            for i in range(len(self.cat_cardinalities)):
                loss_cat += F.cross_entropy(recon_cats[i], x_cat[:, i], reduction='mean')
